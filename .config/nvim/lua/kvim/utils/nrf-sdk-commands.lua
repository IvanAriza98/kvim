...
function nrfConfigBuildProject()
	...
	local zephyrToolchainPath = getConfigField(var.id.NRF_SDK, var.key.NRF_ZEPHYR_TOOLCHAIN_PATH)
	...
	local cmd = "cd " .. zephyrToolchainPath
	cmd = cmd .. " && python -m west build --build-dir " .. appPath .. "/build " .. appPath
	cmd = cmd .. " --pristine --board " .. boardTarget .. " --sysbuild"
	cmd = cmd .. " -- -DCONF_FILE=" .. appPath .. sep .. baseConfigFiles .. " -DBOARD_ROOT=" .. appPath
	...
	if extraDevicetreeOverlays ~= nil and extraDevicetreeOverlays ~= "" then
		local overlayDir = appPath .. sep .. extraDevicetreeOverlays
		local overlayFiles = ""
...
function nrfBuildProject()
	local appPath = getConfigField(var.id.NRF_SDK, var.key.NRF_APP_PATH)
	local zephyrPath = getConfigField(var.id.NRF_SDK, var.key.NRF_ZEPHYR_TOOLCHAIN_PATH)
