...
function nrfFlashProject()
    ...
    return "cd "
		.. projectPath
        .. " && nrfjprog -s " .. snDevice .. " --eraseall"
        .. " && " .. workspacePath .."/.venv/bin/python -m west flash"
		.. " -d " .. applicationPath .. "/build"
		.. " --dev-id " .. snDevice
		.. " --erase"
end
...
function nrfRTTMonitorProject()
	...
	return "cd "
		...
		.. " && nrfjprog -s "
		.. snDevice
		.. " -r && PYTHONIOENCODING=latin1 python -m west rtt -r jlink --build-dir "
		.. applicationPath
		.. "/build -- --speed 1000"
end
